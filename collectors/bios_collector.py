...
class BiosCollector:
    ...
    def collect(self):
        ...
        for system_member in systems["Members"]:
            ...
            if not system_data:
                logging.warning(
                    "Target %s: Cannot get System data from %s",
                    self.col.target,
                    system_url,
                )
                continue
            ...
            if "Bios" not in system_data:
                logging.warning(
                    "Target %s: System %s does not have Bios property",
                    self.col.target,
                    system_url,
                )
                continue
            ...
            if not bios_data:
                logging.warning(
                    "Target %s: Cannot get BIOS data from %s", self.col.target, bios_url
                )
                continue
            ...
            if "Attributes" in bios_data:
                ...
                logging.info(
                    "Target %s: Received %d BIOS attributes.",
                    self.col.target,
                    len(attributes),
                )
                ...
                for attr_name, attr_value in attributes.items():
                    ...
                    self.bios_metrics[metric_name].add_sample(
                        metric_name, value=numeric_value, labels=current_labels
                    )

        # Yield all collected metrics
        logging.info(
            "Target %s: Collected %d BIOS metrics.",
            self.col.target,
            len(self.bios_metrics),
        )
