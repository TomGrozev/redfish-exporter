...
class PerformanceCollector:
    ...
    def get_power_subsystem_metrics(self):
        ...
        if not power_supplies:
            logging.warning(
                "Target %s: No power supplies data received.", self.col.target
            )
            return no_psu_metrics
    ...
    def get_power_supply_metrics(self, power_supply):
        ...
        if not power_supply_data:
            logging.warning(
                "Target %s: No power supply data received.", self.col.target
            )
            return no_psu_metrics
        ...
        if not power_supply_metrics:
            logging.warning(
                "Target %s: No power supply metrics data received.", self.col.target
            )
            return no_psu_metrics
    ...
    def get_old_power_metrics(self):
        ...
        for psu in power_data["PowerSupplies"]:
            ...
            for metric in metrics:
                ...
                no_psu_metrics = False
                power_metric_value = math.nan if psu[metric] is None else psu[metric]
    ...
    def get_temp_metrics(self):
        ...
        if self.col.urls["ThermalSubsystem"]:
            thermal_subsystem = self.col.connect_server(
                self.col.urls["ThermalSubsystem"]
            )
            ...
            if (
                "ThermalMetrics" not in thermal_subsystem
                or "@odata.id" not in thermal_subsystem.get("ThermalMetrics", {})
            ):
                logging.warning(
            ...
            if not result:
                logging.warning(
                    "Target %s: No thermal metrics data received.", self.col.target
                )
                return
